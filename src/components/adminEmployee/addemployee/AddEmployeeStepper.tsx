    ...
    <div className="bg-white p-6 rounded-md shadow-sm">
      ...
      <div className="flex items-center justify-between">
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.number} className="flex items-center flex-1 cursor-pointer" onClick={() => onStepChange(step.number)}>
              <div className="flex items-center">
                <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-sm font-medium ${styles.circle}`}>
                  {step.number}
                </div>
                <div className="ml-3">
                  ...
                  <div className={`text-xs ${styles.statusText}`}>
                    {step.status}
                  </div>
                </div>
              </div>
              ...
              {!isLast && (
                <div className={`flex-1 h-0.5 mx-4 ${index === 0 ? styles.line : 'bg-gray-300'}`}></div>
              )}
            </div>
          );
        })}
